import Link from "next/link";
import Icon from "./Icon";
import BrandLogo from "./BrandLogo";

const COMPANY_LINKS = [
  { href: "/services", label: "Services" },
  { href: "/process", label: "Our Process" },
  { href: "/about", label: "About Us" },
  { href: "/contact", label: "Contact" },
  { href: "/portal", label: "Client Portal" },
];

const LEGAL_LINKS = [
  { href: "/privacy", label: "Privacy Policy" },
  { href: "/faq", label: "FAQ" },
  { href: "/shipping", label: "Shipping & Returns" },
];

export default function Footer() {
  const year = new Date().getFullYear();
  return (
    <footer className="bg-soil text-offwhite/75">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">

          {/* Brand */}
          <div className="sm:col-span-2 lg:col-span-1">
            <Link href="/" className="inline-flex mb-4 text-offwhite">
              <BrandLogo variant="dark" />
            </Link>
            <p className="text-sm leading-relaxed max-w-xs">
              Canterbury&apos;s full-circle commercial food &amp; green-waste recycling.
              Diverting waste from landfill, cutting emissions, and powering local soil.
            </p>
            <p className="mt-5 text-xs font-bold uppercase tracking-widest text-green-leaf">
              Be part of the solution, not the pollution.
            </p>
          </div>

          {/* Company links */}
          <div>
            <h2 className="text-offwhite font-semibold text-xs uppercase tracking-widest mb-4">Company</h2>
            <ul className="space-y-2.5 text-sm">
              {COMPANY_LINKS.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="hover:text-offwhite transition-colors">{label}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h2 className="text-offwhite font-semibold text-xs uppercase tracking-widest mb-4">Get in Touch</h2>
            <ul className="space-y-2.5 text-sm">
              <li>
                <a href="tel:02041841840" className="inline-flex items-center gap-2.5 hover:text-offwhite transition-colors">
                  <Icon name="phone" className="size-4 text-green-leaf/80 shrink-0" />
                  020 4184 1840
                </a>
              </li>
              <li>
                <a href="mailto:info@hungryworms.nz" className="inline-flex items-center gap-2.5 hover:text-offwhite transition-colors">
                  <Icon name="mail" className="size-4 text-green-leaf/80 shrink-0" />
                  info@hungryworms.nz
                </a>
              </li>
              <li className="pt-2">
                <a
                  href="https://www.unclebobs.co.nz/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 rounded-full border border-green-leaf/70 px-3 py-1 text-green-leaf text-xs font-semibold hover:bg-green-leaf hover:text-soil transition-colors"
                >
                  Uncle Bob&apos;s Shop ↗
                </a>
              </li>
            </ul>

            {/* Social */}
            <div className="mt-5 flex gap-3">
              <a
                href="https://www.instagram.com/hungrywormsnz/"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram @hungrywormsnz"
                className="text-offwhite/50 hover:text-offwhite transition-colors"
              >
                <svg className="size-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/hungrywormsnz"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Facebook /hungrywormsnz"
                className="text-offwhite/50 hover:text-offwhite transition-colors"
              >
                <svg className="size-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Legal */}
          <div>
            <h2 className="text-offwhite font-semibold text-xs uppercase tracking-widest mb-4">Legal</h2>
            <ul className="space-y-2.5 text-sm">
              {LEGAL_LINKS.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="hover:text-offwhite transition-colors">{label}</Link>
                </li>
              ))}
            </ul>
          </div>
        </div>

        <div className="mt-10 border-t border-offwhite/10 pt-6 flex flex-col sm:flex-row justify-between gap-2 text-xs text-offwhite/40">
          <span>© {year} Hungry Worms Ltd. Canterbury, New Zealand.</span>
          <span>Powered by nature. Proven by data.</span>
        </div>
      </div>
    </footer>
  );
}
